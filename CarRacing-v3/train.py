...
# Hyperparameters
# ---------------------------------------------------------------------------
N_STEPS       = 4096    # ~4 episodes per rollout; more stable GAE estimates
N_EPOCHS      = 4       # fewer epochs per rollout reduces overfitting on stale data
BATCH_SIZE    = 256     # larger batches for more stable gradient steps
LR            = 3e-4
GAMMA         = 0.99
LAMBDA        = 0.95
CLIP_EPS      = 0.2
...
LOG_INTERVAL  = 5       # log more frequently (rollouts are now longer)
...
def train():
    ...
    # State that persists across resume
    total_steps      = 0
    ...
    while total_steps < TOTAL_STEPS:
        ...
        # ------------------------------------------------------------------
        # Linear LR decay
        frac = max(0.0, 1.0 - total_steps / TOTAL_STEPS)
        for pg in optimizer.param_groups:
            pg['lr'] = LR * frac

        obs_buf  = np.zeros((N_STEPS, agent.N_STACK, agent.IMG_SIZE, agent.IMG_SIZE), dtype=np.float32)
        act_buf  = np.zeros((N_STEPS, agent.N_ACTIONS), dtype=np.float32)
        ...
        for step in range(N_STEPS):
            ...
            with torch.no_grad():
                alpha, beta_param, value = agent.net(obs_tensor)
                dist     = Beta(alpha, beta_param)
                sample   = dist.sample()
                log_prob = dist.log_prob(sample).sum(dim=-1)

            sample_np  = sample.squeeze(0).cpu().numpy()
            env_action = agent._to_env_action(sample_np)  # steering scale + gas/brake fix
        ...
        with torch.no_grad():
            obs_tensor = torch.as_tensor(obs, dtype=torch.float32, device=device).unsqueeze(0)
            _, _, next_value_t = agent.net(obs_tensor)
        ...
        for _ in range(N_EPOCHS):
            ...
            for start in range(0, N_STEPS, BATCH_SIZE):
                ...
                log_probs, values, entropy = agent.evaluate(obs_t[batch_idx], act_t[batch_idx])
        ...
        if rollout_count % LOG_INTERVAL == 0 and episode_returns:
            ...
            print(
                ...
                f"Mean (last 10): {np.mean(recent):>7.1f} | "
                f"Best: {best_mean_return:>7.1f}"
            )
    ...
    # Final resume checkpoint (best model already saved whenever it improved)
    torch.save({
        'model':            agent.state_dict(),
        'optimizer':        optimizer.state_dict(),
        'total_steps':      total_steps,
